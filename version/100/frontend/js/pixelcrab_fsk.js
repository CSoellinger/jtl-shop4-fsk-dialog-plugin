jQuery(document).ready(function ($) {
  ...
  function handleSubmit(form) {
    ...
    if (ajaxSubmit === true) {
      ...
      if ($('#fskPixelCrabModal').length > 0) {
        $('#fskPixelCrabModal').modal('hide');
        $('.modal-overlay').hide();
      }
  ...
  if ($('#fskPixelCrabModal').length > 0) {
    ...
    $('#fskPixelCrabModal').on('click', '.btn-success', function (e) {
      e.preventDefault();
      handleClick($('#fskPixelCrabModal form'));
    });
  }
